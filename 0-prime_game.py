...
round depends only on the parity (odd/even) of the count of primes
≤ n.  If that count is odd Maria (the first player) wins; otherwise Ben
wins.
"""
...
def isWinner(x, nums):
    ...
    maria_wins = 0
    for n in nums[:x]:            # ensure we respect x even if len(nums) > x
        if prime_counts[n] % 2:   # odd → Maria wins this round
            maria_wins += 1
